...
#ifndef BITCOINII_DSP_DSPROOF_H
#define BITCOINII_DSP_DSPROOF_H
...
public:
    ...
    static constexpr size_t DetermineMaxPushDataSize(uint32_t scriptFlags) {
        // In the future, upgrade 11 (May 2025) may allow larger sizes
        (void)scriptFlags;
        return MAX_SCRIPT_ELEMENT_SIZE;
    }
    ...
    //! @param coinsView The coins view to use for UTXO lookups (if nullptr, uses mempool only)
    //! @param spendingTx Optionally pass the spending tx as an optimization; if
    //!                   not specified, it will be looked-up in the mempool.
    //!
    //! Exceptions: None
    //! (implemented in dsproof_validate.cpp)
    Validity validate(const CTxMemPool &mempool, const CCoinsView* coinsView = nullptr,
                      CTransactionRef spendingTx = {}) const EXCLUSIVE_LOCKS_REQUIRED(cs_main);
    ...
    //!    referring to txs in the mempool or confirmed in the blockchain).
    ...
    static bool checkIsProofPossibleForAllInputsOfTx(const CTxMemPool &mempool, const CCoinsView* coinsView,
                                                     const CTransaction &tx,
                                                     bool *pProtected = nullptr) EXCLUSIVE_LOCKS_REQUIRED(cs_main);

    const uint256 & prevTxId() const { return m_outPoint.hash; }
    uint32_t prevOutIndex() const { return m_outPoint.n; }
...
private:
    ...
    /// - any pushData size >520 bytes (or limit for current scriptFlags)
