 ...
 */
static const char* BITCOINII_PID_FILENAME = "bitcoincashIId.pid";
/**
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-doublespendproof", strprintf("Enable double-spend proof creation and relay (default: %u)", 1), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    // https://github.com/bitcoinII/bitcoinII/pull/23542 have become widespread.
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    DoubleSpendProof::SetEnabled(args.GetBoolArg("-doublespendproof", true));
    if (DoubleSpendProof::IsEnabled()) {
        LogPrintf("Double-spend proof subsystem enabled\n");
    }
    ...
    for (bool fLoaded = false; !fLoaded && !ShutdownRequested(node);) {
        ...
        if (DoubleSpendProof::IsEnabled()) {
            node.scheduler->scheduleEvery([&node] {
                if (node.mempool) {
                    return node.mempool->doubleSpendProofStorage().periodicCleanup();
                }
                return false;
