...
void ArgsManager::AddArg(const std::string& name, const std::string& help, unsigned int flags, const OptionsCategory& cat)
{
    ...
    // This fixes test isolation issues where gArgs may not be fully cleared between tests.
    // In production, SetupServerArgs is only called once, so this is only relevant for tests.
    (void)ret; // Silence unused variable warning if assert is disabled
