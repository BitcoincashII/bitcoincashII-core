...
// Rejects bech32/witness addresses after fork height 53200
static void ValidateDestinationsForFork(const CWallet& wallet, const std::vector<std::pair<CTxDestination, CAmount>>& outputs)
{
    LOCK(wallet.cs_wallet);
    int nHeight = wallet.GetLastBlockHeight();
        ...
        for (const auto& [destination, amount] : outputs) {
            if (IsWitnessDestination(destination)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                    "Bech32/SegWit addresses are not allowed after BCH2 fork (block 53200). "
                    "Use CashAddr format (bitcoincashii:...) instead.");
            }
        }
    }
}
...
static UniValue FinishTransaction(const std::shared_ptr<CWallet> pwallet, const UniValue& options, const CMutableTransaction& rawTx)
{
    ...
    int sighash_type = SIGHASH_ALL;
    {
        ...
        const Consensus::Params& consensusParams = Params().GetConsensus();
        // Use height+1: tx will be mined in the NEXT block
        if (consensusParams.IsBCH2ForkActive(nHeight + 1)) {
            sighash_type |= SIGHASH_FORKID;
        }
    }

    CMutableTransaction mtx;
    complete = FinalizeAndExtractPSBT(psbtx, mtx, sighash_type);
...
RPCHelpMan sendtoaddress()
{
    return RPCHelpMan{"sendtoaddress",
        ...
        HELP_REQUIRING_PASSPHRASE,
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The BCH2 address to send to."},
                ...
                RPCExamples{
                    ...
                    "\nSend 0.1 BCH2 with a confirmation target of 6 blocks in economical fee estimate mode using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"donation\" \"sean's outpost\" false true 6 economical") +
                    "\nSend 0.1 BCH2 with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB, subtract fee from amount, BIP125-replaceable, using positional arguments\n"
                    + HelpExampleCli("sendtoaddress", "\"" + EXAMPLE_ADDRESS[0] + "\" 0.1 \"drinks\" \"room77\" true true null \"unset\" null 1.1") +
                    "\nSend 0.2 BCH2 with a confirmation target of 6 blocks in economical fee estimate mode using named arguments\n"
                    + HelpExampleCli("-named sendtoaddress", "address=\"" + EXAMPLE_ADDRESS[0] + "\" amount=0.2 conf_target=6 estimate_mode=\"economical\"") +
                    "\nSend 0.5 BCH2 with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    auto parsed_outputs = ParseOutputs(address_amounts);
    ValidateDestinationsForFork(*pwallet, parsed_outputs);
    std::vector<CRecipient> recipients = CreateRecipients(
            std::move(parsed_outputs),
        ...
        HELP_REQUIRING_PASSPHRASE,
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::NO, "The addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The BCH2 address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    auto parsed_outputs = ParseOutputs(sendTo);
    ValidateDestinationsForFork(*pwallet, parsed_outputs);
    std::vector<CRecipient> recipients = CreateRecipients(
            std::move(parsed_outputs),
...
CreatedTransactionResult FundTransaction(CWallet& wallet, const CMutableTransaction& tx, const std::vector<CRecipient>& recipients, const UniValue& options, CCoinControl& coinControl, bool override_min_fee)
{
      ...
      if (options.type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress") || options.exists("change_address")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Change address must be a valid BCH2 address");
            }

            // BCH2: reject witness change addresses after fork — change sent to SegWit is unspendable
            if (Params().GetConsensus().IsBCH2ForkActive(WITH_LOCK(wallet.cs_wallet, return wallet.GetLastBlockHeight())) && IsWitnessDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                    "Bech32/SegWit change addresses are not allowed after BCH2 fork. Use CashAddr format.");
            }
    ...
    if (options.exists("solving_data")) {
        ...
        if (solving_data.exists("pubkeys")) {
            for (const UniValue& pk_univ : solving_data["pubkeys"].get_array().getValues()) {
                ...
                // Add witness script for pubkeys (only pre-fork; post-fork SegWit is disabled)
                if (!Params().GetConsensus().IsBCH2ForkActive(WITH_LOCK(wallet.cs_wallet, return wallet.GetLastBlockHeight()))) {
    ...
    if (options.exists("input_weights")) {
        for (const UniValue& input : options["input_weights"].get_array().getValues()) {
            ...
            CHECK_NONFATAL(min_input_weight == 42);
            if (weight < min_input_weight) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, weight cannot be less than 42 (41 bytes (size of outpoint + sequence + empty scriptSig) + 1 (empty witness))");
            }
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::OMITTED, "For backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        Cat<std::vector<RPCArg>>(
                        {
                            ...
                            {"changeAddress", RPCArg::Type::STR, RPCArg::DefaultHint{"automatic"}, "The BCH2 address to receive the change"},
                            {"changePosition", RPCArg::Type::NUM, RPCArg::DefaultHint{"random"}, "The index of the change output"},
                            {"change_type", RPCArg::Type::STR, RPCArg::DefaultHint{"set by -changetype"}, "The output type to use. Only valid if changeAddress is not specified. Only \"legacy\" is supported after BCH2 fork (SegWit is disabled)."},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    ValidateDestinationsForFork(*pwallet, destinations);
...
static std::vector<RPCArg> OutputsDoc()
{
    return
    {
        {"", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::OMITTED, "",
            {
                {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the BCH2 address,\n"
        ...
        [want_psbt](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // BCH2: RBF (Replace-By-Fee) is not supported after the fork.
    // bumpfee would create a conflicting transaction that mempool nodes reject.
    {
        LOCK(pwallet->cs_wallet);
        const Consensus::Params& consensusParams = Params().GetConsensus();
        if (consensusParams.IsBCH2ForkActive(pwallet->GetLastBlockHeight())) {
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                "Fee bumping (RBF) is not supported on BCH2. "
                "Transactions cannot be replaced after BCH2 fork.");
        }
    }
...
RPCHelpMan send()
{
    return RPCHelpMan{"send",
        ...
        "\nSend a transaction.\n",
        {
            ...
            {"options", RPCArg::Type::OBJ_NAMED_PARAMS, RPCArg::Optional::OMITTED, "",
                Cat<std::vector<RPCArg>>(
                {
                    ...
                    {"change_address", RPCArg::Type::STR, RPCArg::DefaultHint{"automatic"}, "The BCH2 address to receive the change"},
                    {"change_position", RPCArg::Type::NUM, RPCArg::DefaultHint{"random"}, "The index of the change output"},
                    {"change_type", RPCArg::Type::STR, RPCArg::DefaultHint{"set by -changetype"}, "The output type to use. Only valid if change_address is not specified. Only \"legacy\" is supported after BCH2 fork (SegWit is disabled)."},
        ...
        RPCExamples{""
        "\nSend 0.1 BCH2 with a confirmation target of 6 blocks in economical fee estimate mode\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.1}' 6 economical\n") +
        "Send 0.2 BCH2 with a fee rate of 1.1 " + CURRENCY_ATOM + "/vB using positional arguments\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" 1.1\n") +
        "Send 0.2 BCH2 with a fee rate of 1 " + CURRENCY_ATOM + "/vB using the options argument\n"
        + HelpExampleCli("send", "'{\"" + EXAMPLE_ADDRESS[0] + "\": 0.2}' null \"unset\" null '{\"fee_rate\": 1}'\n") +
        "Send 0.3 BCH2 with a fee rate of 25 " + CURRENCY_ATOM + "/vB using named arguments\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            auto parsed_outputs = ParseOutputs(outputs);
            ValidateDestinationsForFork(*pwallet, parsed_outputs);
            std::vector<CRecipient> recipients = CreateRecipients(
                    std::move(parsed_outputs),
    ...
    return RPCHelpMan{"sendall",
        ...
        "If your wallet contains many small inputs, either because it received tiny payments or as a result of accumulating change, consider using `send_max` to exclude inputs that are worth less than the fees needed to spend them.\n",
        {
            {"recipients", RPCArg::Type::ARR, RPCArg::Optional::NO, "The sendall destinations. Each address may only appear once.\n"
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "A BCH2 address which receives an equal share of the unspecified amount."},
                    {"", RPCArg::Type::OBJ_USER_KEYS, RPCArg::Optional::OMITTED, "",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the BCH2 address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                        },
                    },
                },
            },
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            {
                std::vector<std::pair<CTxDestination, CAmount>> destinations;
                for (const auto& tx_out : rawTx.vout) {
                    CTxDestination dest;
                    ExtractDestination(tx_out.scriptPubKey, dest);
                    destinations.emplace_back(dest, tx_out.nValue);
                }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (complete) {
        ...
        CHECK_NONFATAL(FinalizeAndExtractPSBT(psbtx, mtx, nHashType));
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    auto parsed_outputs = ParseOutputs(outputs);
    ValidateDestinationsForFork(wallet, parsed_outputs);
    std::vector<CRecipient> recipients = CreateRecipients(
            std::move(parsed_outputs),
