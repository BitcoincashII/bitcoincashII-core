...
/// compatibility. After fork activation, witness addresses cannot be created for new outputs.
...
public:
    ...
    std::string operator()(const PKHash& id) const
    {
        ...
        return cashaddr::Encode("bitcoincashii", payload);
    }
...
CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, std::string& error_str, std::vector<int>* error_locations)
{
    ...
    // Try CashAddr decoding first (bitcoincashii: prefix)
    const std::string cashaddr_prefix = "bitcoincashii";
    ...
    if (is_cashaddr || (str.find(':') != std::string::npos && ToLower(str.substr(0, str.find(':'))) == cashaddr_prefix)) {
        ...
        if (!decoded.first.empty() && !decoded.second.empty()) {
            ...
            uint8_t type = unpacked.first;
            std::vector<uint8_t> hash_data = unpacked.second;
            if (hash_data.size() == 20) {
                uint160 hash;
                std::copy(hash_data.begin(), hash_data.end(), hash.begin());
                if (type == 0) {
                    return PKHash(hash);
                } else if (type == 1) {
