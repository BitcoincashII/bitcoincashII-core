...
static RPCHelpMan generatetodescriptor()
{
    return RPCHelpMan{
        ...
        "Mine to a specified descriptor and return the block hashes.",
        {
            ...
            {"descriptor", RPCArg::Type::STR, RPCArg::Optional::NO, "The descriptor to send the newly generated BCH2 to."},
        ...
        "Mine to a specified address and return the block hashes.",
         {
             ...
             {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated BCH2 to."},
             {"maxtries", RPCArg::Type::NUM, RPCArg::Default{DEFAULT_MAX_TRIES}, "How many iterations to try."},
         },
         RPCResult{
             RPCResult::Type::ARR, "", "hashes of blocks generated",
         ...
         RPCExamples{
            ...
            + "If you are using the " PACKAGE_NAME " wallet, you can get a new address to send the newly generated BCH2 to with:\n"
...
static RPCHelpMan generateblock()
{
    return RPCHelpMan{"generateblock",
        ...
        {
            {"output", RPCArg::Type::STR, RPCArg::Optional::NO, "The address or descriptor to send the newly generated BCH2 to."},
...
// NOTE: Unlike wallet RPC (which use BCH2 values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
static RPCHelpMan prioritisetransaction()
{
    return RPCHelpMan{"prioritisetransaction",
            ...
            "                  DEPRECATED. For forward compatibility use named arguments and omit this parameter."},
                    {"fee_delta", RPCArg::Type::NUM, RPCArg::Optional::NO, "The fee value (in satooshis) to add (or subtract, if negative).\n"
        ...
        RPCResult{
            RPCResult::Type::OBJ_DYN, "", "prioritisation keyed by txid",
            {
                {RPCResult::Type::OBJ, "<transactionid>", "", {
                    {RPCResult::Type::NUM, "fee_delta", "transaction fee delta in satooshis"},
                    {RPCResult::Type::BOOL, "in_mempool", "whether this transaction is currently in mempool"},
                    {RPCResult::Type::NUM, "modified_fee", /*optional=*/true, "modified fee in satooshis. Only returned if in_mempool=true"},
                }}
            },
        },
        RPCExamples{
            ...
            RPCResult{"Otherwise", RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::ARR, "transactions", "contents of non-coinbase transactions that should be included in the next block",
                {
                    {RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::NUM, "fee", "difference in value between transaction inputs and outputs (in satooshis); for coinbase transactions, this is a negative Number of the total collected block fees (ie, not including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't one"},
                ...
                {RPCResult::Type::NUM, "coinbasevalue", "maximum allowable input to coinbase transaction, including the generation award and transaction fees (in satooshis)"},
                ...
                {RPCResult::Type::STR_HEX, "default_witness_commitment", /*optional=*/true, "a valid witness commitment for the unmodified block template (not present after BCH2 fork)"},
            }},
        },
        RPCExamples{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // GBT must be called with 'segwit' set in the rules (only before BCH2 fork)
    // After BCH2 fork, SegWit is disabled so we don't require it
    int nNextHeight = active_chain.Tip()->nHeight + 1;
    if (!consensusParams.IsBCH2ForkActive(nNextHeight) && setClientRules.count("segwit") != 1) {
    ...
    const bool fPreSegWit = !consensusParams.IsSegwitActive(nNextHeight);
    ...
    if (!fPreSegWit && !consensusParams.IsBCH2ForkActive(pindexPrev->nHeight + 1)) {
    ...
    // Note: nNextHeight already declared earlier for BCH2 fork check
    bool fBCH2Active = consensusParams.IsBCH2ForkActive(nNextHeight);
    int64_t nSigOpLimit = fBCH2Active ? BCH2_MAX_BLOCK_SIGOPS_COST : BCH2_LEGACY_MAX_BLOCK_SIGOPS_COST;
    int64_t nSizeLimit = fBCH2Active ? BCH2_MAX_BLOCK_SIZE : BCH2_LEGACY_MAX_BLOCK_SIZE;
    if (fPreSegWit && !fBCH2Active) {
    ...
    if (!fPreSegWit && !fBCH2Active) {
        result.pushKV("weightlimit", (int64_t)BCH2_LEGACY_MAX_BLOCK_SIZE);
    } else if (fBCH2Active) {
        result.pushKV("weightlimit", (int64_t)BCH2_MAX_BLOCK_SIZE);
    }
