 ...
 *
 * field. Token data is serialized after scriptPubKey with prefix 0xef.
 */
class CTxOut
{
public:
    ...
    CTxOut(const CAmount& nValueIn, CScript scriptPubKeyIn, const OutputToken& token);
    ...
    bool HasTokenData() const { return static_cast<bool>(tokenData); }

    const OutputToken* GetTokenData() const { return tokenData.get(); }

    friend bool operator==(const CTxOut& a, const CTxOut& b)
    {
        return (a.nValue       == b.nValue &&
                a.scriptPubKey == b.scriptPubKey &&
                a.tokenData    == b.tokenData);
    }
...
// and parsing pre-fork transactions. After BCH2 fork, new transactions have no witness data.
