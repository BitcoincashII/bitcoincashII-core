...
#ifndef BITCOINII_SEEDER_DB_H
#define BITCOINII_SEEDER_DB_H
...
private:
    ...
    // Helper to serialize float as its raw bytes (platform-specific, but consistent for same platform)
    static void SerializeFloat(DataStream& s, float f) {
        uint32_t bits;
        static_assert(sizeof(float) == sizeof(uint32_t));
        std::memcpy(&bits, &f, sizeof(bits));
        s << bits;
    }
    static void UnserializeFloat(DataStream& s, float& f) {
        uint32_t bits;
        s >> bits;
        static_assert(sizeof(float) == sizeof(uint32_t));
...
public:
    ...
    void Serialize(Stream& s) const {
        // Serialize floats as uint32_t to work around BCH2 not supporting float serialization
        uint32_t w, c, r;
        static_assert(sizeof(float) == sizeof(uint32_t));
        std::memcpy(&w, &weight, sizeof(w));
        std::memcpy(&c, &count, sizeof(c));
        std::memcpy(&r, &reliability, sizeof(r));
        s << w << c << r;
    }
    ...
    void Unserialize(Stream& s) {
        ...
        std::memcpy(&count, &c, sizeof(count));
        std::memcpy(&reliability, &r, sizeof(reliability));
    }
...
public:
    ...
    template <typename Stream> void Serialize(Stream &s_) const {
        ...
        // Wrap stream with params for CNetAddr serialization
        // Use V2 for new data to support all address types
    ...
    template <typename Stream> void Unserialize(Stream &s_) {
        ...
        // Wrap stream with params for CNetAddr deserialization
        // Use V1 for compatibility with old data, then V2 for new
        // Read version first to determine encoding
        int nVersion;
        s_ >> nVersion;

        // Use V2 encoding for version >= firstAddrV2Version
        const auto ser_params = (nVersion >= firstAddrV2Version ? CNetAddr::V2 : CNetAddr::V1);
