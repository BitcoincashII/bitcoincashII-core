...
const QString BITCOINII_IPC_PREFIX("bch2:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOINII_IPC_PREFIX, Qt::CaseInsensitive)) // bch2: URI
        {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("bch2://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'bch2://' is not a valid URI. Use 'bch2:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOINII_IPC_PREFIX, Qt::CaseInsensitive)) // bch2: URI
    {
