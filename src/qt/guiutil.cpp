...
bool parseBitcoinIIURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!BitcoinIIUnits::parse(BitcoinIIUnit::BCH2, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
QString formatBitcoinIIURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinIIUnits::format(BitcoinIIUnit::BCH2, info.amount, false, BitcoinIIUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "bitcoincashII.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("bitcoincashII-%s.desktop", ChainTypeToString(chain)));
}
