...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // After BCH2 fork, SegWit is disabled and witness data is not allowed
    // Use Height()+1 because this tx will be included in the NEXT block
    const Consensus::Params& consensusParams = m_active_chainstate.m_chainman.GetConsensus();
    const int nextBlockHeight = m_active_chainstate.m_chain.Height() + 1;
    if (consensusParams.IsBCH2ForkActive(nextBlockHeight)) {
        if (tx.HasWitness()) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "segwit-disabled",
                "SegWit transactions are not allowed after BCH2 fork");
        }
    }
    ...
    // Transactions smaller than the minimum non-witness size are not relayed to mitigate CVE-2017-12842.
    // BCH Magnetic Anomaly reduced the minimum from 100 to 65 bytes; BCH2 inherits this post-fork.
    const unsigned int minTxSize = consensusParams.IsBCH2ForkActive(nextBlockHeight)
        ? BCH2_MIN_STANDARD_TX_SIZE
        : MIN_STANDARD_TX_NONWITNESS_SIZE;
    if (::GetSerializeSize(TX_NO_WITNESS(tx)) < minTxSize)
        ...
        if (ptxConflicting) {
            if (!args.m_allow_replacement) {
                ...
                if (DoubleSpendProof::IsEnabled()) {
                    try {
                        Coin coin;
                        if (m_active_chainstate.CoinsTip().GetCoin(txin.prevout, coin)) {
                            // Use correct script flags for the active chain
                            const bool bch2_active_for_dsp = consensusParams.IsBCH2ForkActive(nextBlockHeight);
                            const unsigned int dsp_flags2 = bch2_active_for_dsp
                                ? BCH2_SCRIPT_VERIFY_FLAGS
                                : STANDARD_SCRIPT_VERIFY_FLAGS;
                            auto dsproof = DoubleSpendProof::create(
                                dsp_flags2,
                                tx,
                                *ptxConflicting,
                                txin.prevout,
                            ...
                            if (m_pool.doubleSpendProofStorage().add(dsproof)) {
                                LogPrint(BCLog::MEMPOOL, "DSProof created for double-spend on %s:%d, dspid=%s\n",
                                         txin.prevout.hash.ToString(), txin.prevout.n,
                                         dsproof.GetId().ToString());
                            }
                        }
                    } catch (const std::exception& e) {
                        LogPrint(BCLog::MEMPOOL, "DSProof creation failed: %s\n", e.what());
                    }
                }
            ...
            if (!ws.m_conflicts.count(ptxConflicting->GetHash()))
            {
                ...
                // BCH2: RBF is disabled after fork — all transactions are first-seen-safe.
                // DSProof is used instead to notify merchants of double-spend attempts.
                const bool bch2_fork_active = consensusParams.IsBCH2ForkActive(nextBlockHeight);
                const bool allow_rbf = bch2_fork_active ? false : (m_pool.m_full_rbf || SignalsOptInRBF(*ptxConflicting) || ptxConflicting->nVersion == 3);
                if (!allow_rbf) {
                    if (DoubleSpendProof::IsEnabled()) {
                        try {
                            // Get the coin being double-spent for signature verification
                            Coin coin;
                            if (m_active_chainstate.CoinsTip().GetCoin(txin.prevout, coin)) {
                                // Create DSProof — use correct script flags for the active chain
                                const unsigned int dsp_flags = bch2_fork_active
                                    ? BCH2_SCRIPT_VERIFY_FLAGS
                                    : STANDARD_SCRIPT_VERIFY_FLAGS;
                                auto dsproof = DoubleSpendProof::create(
                                    dsp_flags,
                            ...
                            // DSProof creation may fail for non-P2PKH inputs or other reasons
                            LogPrint(BCLog::MEMPOOL, "DSProof creation failed: %s\n", e.what());
                        }
                    }
    ...
    if (consensusParams.IsCashTokensActive(m_active_chainstate.m_chain.Height() + 1)) {
        // Build vector of spent outputs for token validation
        std::vector<CTxOut> spentOutputs;
        spentOutputs.reserve(tx.vin.size());
        for (const CTxIn& txin : tx.vin) {
            const Coin& coin = m_view.AccessCoin(txin.prevout);
            spentOutputs.push_back(coin.out);
        }
        TokenValidationResult tokenResult = CheckTokens(tx, spentOutputs, true);
        if (!tokenResult) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, tokenResult.error);
        }
    }
    ...
    // BCH2: Use fork-aware flags for sigop counting. Post-fork, we must NOT
    // include SCRIPT_VERIFY_WITNESS, otherwise spending pre-fork UTXOs with
    // witness-program-like scriptPubKeys inflates the sigop count (and thus
    // virtual size) compared to ConnectBlock, which uses GetBlockScriptFlags.
    const unsigned int sigOpFlags = consensusParams.IsBCH2ForkActive(nextBlockHeight)
        ? BCH2_SCRIPT_VERIFY_FLAGS
        : STANDARD_SCRIPT_VERIFY_FLAGS;
    int64_t nSigOpsCost = GetTransactionSigOpCost(tx, m_view, sigOpFlags);
    ...
    // The only exception is v3 transactions (not applicable on BCH2 post-fork).
    const bool is_v3_exempt = !m_active_chainstate.m_chainman.GetConsensus().IsBCH2ForkActive(m_active_chainstate.m_chain.Height() + 1) && ws.m_ptx->nVersion == 3;
    if (!bypass_limits && !is_v3_exempt && ws.m_modified_fees < m_pool.m_min_relay_feerate.GetFee(ws.m_vsize)) {
    ...
    // BCH2: Skip V3/TRUC policy checks post-fork — BCH2 has no V3 transaction concept
    if (!m_active_chainstate.m_chainman.GetConsensus().IsBCH2ForkActive(m_active_chainstate.m_chain.Height() + 1)) {
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    const Consensus::Params& consensusParams = m_active_chainstate.m_chainman.GetConsensus();
    const int nextBlockHeight = m_active_chainstate.m_chain.Height() + 1;
    const unsigned int scriptVerifyFlags = consensusParams.IsBCH2ForkActive(nextBlockHeight)
        ? BCH2_SCRIPT_VERIFY_FLAGS
        : STANDARD_SCRIPT_VERIFY_FLAGS;
...
bool MemPoolAccept::ConsensusScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    // BCH2: At the fork boundary, the tip is pre-fork but the next block needs
    // post-fork flags (SIGHASH_FORKID, NO_SEGWIT). Use next-block height to
    // determine the correct flags, otherwise valid BCH2 txs are rejected.
    const Consensus::Params& csParams = m_active_chainstate.m_chainman.GetConsensus();
    const int nextHeight = m_active_chainstate.m_chain.Height() + 1;
    unsigned int currentBlockScriptVerifyFlags;
    if (csParams.IsBCH2ForkActive(nextHeight) && !csParams.IsBCH2ForkActive(m_active_chainstate.m_chain.Height())) {
        // Fork boundary: tip is pre-fork, next block is post-fork
        currentBlockScriptVerifyFlags = BCH2_SCRIPT_VERIFY_FLAGS;
    } else {
        currentBlockScriptVerifyFlags = GetBlockScriptFlags(*m_active_chainstate.m_chain.Tip(), m_active_chainstate.m_chainman);
    }
...
bool CScriptCheck::operator()() {
    ...
    // Create ScriptExecutionContext for introspection opcodes (OP_INPUTINDEX, OP_UTXOVALUE, etc.)
    // Uses m_spent_outputs from PrecomputedTransactionData when available; falls back to limited context.
    std::optional<ScriptExecutionContext> execContext;
    if (txdata->m_spent_outputs_ready) {
        execContext.emplace(nIn, *ptxTo, txdata->m_spent_outputs);
    } else {
        execContext.emplace(nIn, *ptxTo, m_tx_out);
    }
    return VerifyScript(scriptSig, m_tx_out.scriptPubKey, witness, nFlags,
                        CachingTransactionSignatureChecker(ptxTo, nIn, m_tx_out.nValue, cacheStore, *txdata, &*execContext), &error);
}
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // Under CTOR ordering, transactions are sorted by txid, not topologically.
    // A child tx may appear before its parent in the block. We must restore ALL
    // inputs first, then remove ALL outputs, to handle intra-block UTXO deps.
    const bool fCTOR = m_chainman.GetConsensus().IsMagneticAnomalyActive(pindex->nHeight);
    if (fCTOR) {
        // Pass 1: Restore all inputs (skip coinbase at index 0)
        ...
        // Pass 2: Revert all created outputs
        ...
        // Pre-CTOR: Original Bitcoin Core single-pass approach (reverse iteration)
...
static unsigned int GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    if (consensusparams.IsBCH2ForkActive(block_index.nHeight)) {
        // BCH2 post-fork: P2SH enabled, SegWit/Taproot disabled, SIGHASH_FORKID required
        uint32_t flags{SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_SIGHASH_FORKID | SCRIPT_VERIFY_NO_SEGWIT};

        // All standard signature rules (mandatory in BCH)
        flags |= SCRIPT_VERIFY_DERSIG;
        flags |= SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY;
        flags |= SCRIPT_VERIFY_CHECKSEQUENCEVERIFY;
        flags |= SCRIPT_VERIFY_NULLDUMMY;  // Clean stack rule
        flags |= SCRIPT_VERIFY_NULLFAIL;  // Failed sigs must be empty (BCH mandatory since Magnetic Anomaly)

        // BCH mandatory flags (these are just "standard" in BTC but mandatory in BCH)
        flags |= SCRIPT_VERIFY_STRICTENC;    // Strict signature encoding
        flags |= SCRIPT_VERIFY_LOW_S;        // Low S values in signatures
        flags |= SCRIPT_VERIFY_MINIMALDATA;  // Minimal push operations
        flags |= SCRIPT_VERIFY_SIGPUSHONLY;  // Only push ops in scriptSig
        flags |= SCRIPT_VERIFY_CLEANSTACK;   // Clean stack after execution

        // Note: We use block_index.nHeight for the "previous" block since flags
        // apply to the block being validated, which is at height block_index.nHeight + 1
        // But GetBlockScriptFlags receives the block being validated, so we use nHeight directly

        // Magnetic Anomaly (Nov 2018): OP_CHECKDATASIG, byte manipulation opcodes
        // Enable OP_CAT, OP_SPLIT, OP_AND, OP_OR, OP_XOR, OP_DIV, OP_MOD
        if (consensusparams.IsMagneticAnomalyActive(block_index.nHeight)) {
            flags |= SCRIPT_ENABLE_BCH2_OPCODES;
            flags |= SCRIPT_ENABLE_BITWISE_OPCODES;
            flags |= SCRIPT_ENABLE_ARITHMETIC_OPCODES;
        }

        // Graviton (Nov 2019): Schnorr signatures in OP_CHECKSIG/OP_CHECKDATASIG
        if (consensusparams.IsGravitonActive(block_index.nHeight)) {
            flags |= SCRIPT_ENABLE_SCHNORR;
        }

        // Phonon (May 2020): OP_REVERSEBYTES, SigChecks
        if (consensusparams.IsPhononActive(block_index.nHeight)) {
            flags |= SCRIPT_ENABLE_REVERSEBYTES;
        }

        // Upgrade 8 (May 2022): Native introspection, 64-bit integers
        if (consensusparams.IsUpgrade8Active(block_index.nHeight)) {
            flags |= SCRIPT_ENABLE_INTROSPECTION;
        }

        // Upgrade 9 (May 2023): CHIP limits, P2SH32, CashTokens
        // P2SH32 and CashTokens are validated at the transaction level,
        // not script interpreter level, so no flags needed here.

        // Upgrade 10 (May 2024): VM Limits, BigInt math
        if (consensusparams.IsUpgrade10Active(block_index.nHeight)) {
            flags |= SCRIPT_ENABLE_VM_LIMITS;
        }

        // Upgrade 11 (May 2025): ABLA - no new script flags

        // Upgrade 12 (May 2026): Shift opcodes (OP_LSHIFT, OP_RSHIFT)
        if (consensusparams.IsUpgrade12Active(block_index.nHeight)) {
    ...
    // Pre-fork: Standard Bitcoin Core behavior with SegWit
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // IMPORTANT: txsdata must be declared before control so that it is
    // destroyed after control's destructor waits for worker threads.
    ...
    // When CTOR is active, transactions are ordered by txid, not topologically.
    // A child tx may appear before its parent. We must add ALL outputs to the
    // UTXO view first, then validate inputs and spend coins in a second pass.
    const bool fCTOR = params.GetConsensus().IsMagneticAnomalyActive(pindex->nHeight);
    if (fCTOR) {
        try {
            for (const auto& ptx : block.vtx) {
            ...
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
                "tx-duplicate", "ConnectBlock(): tried to overwrite transaction");
        }
    }
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (params.GetConsensus().IsCashTokensActive(pindex->nHeight)) {
                std::vector<CTxOut> spentOutputs;
                spentOutputs.reserve(tx.vin.size());
                for (const CTxIn& txin : tx.vin) {
                    const Coin& coin = view.AccessCoin(txin.prevout);
                ...
                TokenValidationResult tokenResult = CheckTokens(tx, spentOutputs, true);
                if (!tokenResult) {
                    state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, tokenResult.error);
                    return error("%s: CheckTokens: %s, %s", __func__, tx.GetHash().ToString(), tokenResult.error);
                }
            }
        ...
        int64_t nMaxSigOpsCost = params.GetConsensus().IsBCH2ForkActive(pindex->nHeight) ? BCH2_MAX_BLOCK_SIGOPS_COST : BCH2_LEGACY_MAX_BLOCK_SIGOPS_COST;
        if (nSigOpsCost > nMaxSigOpsCost) {
            LogPrintf("ERROR: ConnectBlock(): too many sigops (max=%lld)\n", nMaxSigOpsCost);
        ...
        if (fCTOR) {
            // CTOR two-pass: outputs already added in Pass 1, only spend inputs here
            if (!tx.IsCoinBase()) {
            ...
            // Pre-CTOR: single-pass add outputs + spend inputs (original Bitcoin Core path)
            CTxUndo undoDummy;
            if (i > 0) {
                blockundo.vtxundo.emplace_back();
            }
    ...
    // ABLA (Upgrade 11): Compute and cache the ABLA state for this block.
    // This state is used by ContextualCheckBlock to determine the block size limit
    // for the next block.
    const Consensus::Params& ablaConsensus = params.GetConsensus();
    if (ablaConsensus.IsUpgrade11Active(pindex->nHeight)) {
        abla::Config ablaConfig = abla::Config::MakeDefault(ablaConsensus.nDefaultConsensusBlockSize);
        pindex->nBlockSize = ::GetSerializeSize(TX_NO_WITNESS(block));

        abla::State prevState;
        if (pindex->pprev && pindex->pprev->nABLAControlBlockSize > 0) {
            // Use cached state from previous block
            prevState = abla::State::FromTuple({pindex->pprev->nBlockSize,
                                                 pindex->pprev->nABLAControlBlockSize,
                                                 pindex->pprev->nABLAElasticBufferSize});
        } else {
            // First block after ABLA activation or no cached state: use initial state
            prevState = abla::State(ablaConfig, 0);
        }

        abla::State newState = prevState.NextBlockState(ablaConfig, pindex->nBlockSize);
        pindex->nABLAControlBlockSize = newState.GetControlBlockSize();
        pindex->nABLAElasticBufferSize = newState.GetElasticBufferSize();
    }
...
void Chainstate::UpdateTip(const CBlockIndex* pindexNew)
{
    ...
    if (!m_chainman.IsInitialBlockDownload()) {
        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
            // Skip versionbits used by known disabled deployments (Taproot bit 2, TESTDUMMY bit 28)
            // BC2 blocks signaled Taproot before the BCH2 fork, triggering false warnings
            if (bit == 2 || bit == 28) continue;
...
bool Chainstate::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    // BCH2/BCHN: Update automatic finalization (rolling checkpoints)
    UpdateAutomaticFinalization(pindexNew);
...
bool Chainstate::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    // BCH2/BCHN: Check if this reorg would cross the finalization boundary
    if (WouldCrossFinalization(pindexFork)) {
        // This reorg would disconnect finalized blocks - reject it
        LogPrintf("BCH2: Rejecting reorg that would cross finalization at height %d (fork at %d)\n",
                  m_finalized_height, pindexFork ? pindexFork->nHeight : -1);

        // Park the competing chain tip that triggered this reorg attempt
        if (pindexMostWork && !pindexMostWork->IsParked()) {
            pindexMostWork->nStatus |= BLOCK_PARKED;
            m_blockman.m_dirty_blockindex.insert(pindexMostWork);
            setBlockIndexCandidates.erase(pindexMostWork);
        }

        // Mark state as rejected due to finalization
        state.Invalid(BlockValidationResult::BLOCK_CACHED_INVALID,
                      "bad-fork-finalized",
                      "Reorg rejected: would disconnect finalized blocks");
        fInvalidFound = true;
        return true;  // Return true but with fInvalidFound set
    }
...
// =========================================================================
// BCH2/BCHN Automatic Finalization (Rolling Checkpoints) Implementation
// =========================================================================
...
bool Chainstate::ParkBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    ...
    // Genesis block can't be parked
    assert(pindex);
    if (pindex->nHeight == 0) return false;

    CBlockIndex* to_mark_parked = pindex;
    bool pindex_was_in_chain = false;
    int disconnected = 0;

    // We do not allow ActivateBestChain() to run while ParkBlock() is
    // running, as that could cause the tip to change while we disconnect
    // blocks.
    LOCK(m_chainstate_mutex);

    // Build a map of candidate blocks by work
    std::multimap<const arith_uint256, CBlockIndex*> candidate_blocks_by_work;
    ...
    // Disconnect (descendants of) pindex, and mark them as parked.
    while (true) {
        if (m_chainman.m_interrupt) break;
        ...
        if (!m_chain.Contains(pindex)) break;
        pindex_was_in_chain = true;
        CBlockIndex* parked_walk_tip = m_chain.Tip();

        DisconnectedBlockTransactions disconnectpool{MAX_DISCONNECTED_TX_POOL_BYTES};
        bool ret = DisconnectTip(state, &disconnectpool);
        MaybeUpdateMempoolForReorg(disconnectpool, (++disconnected <= 10) && ret);
        if (!ret) return false;
        assert(parked_walk_tip->pprev == m_chain.Tip());

        // Mark disconnected blocks as parked
        parked_walk_tip->nStatus |= BLOCK_PARKED;
        m_blockman.m_dirty_blockindex.insert(parked_walk_tip);
        setBlockIndexCandidates.erase(parked_walk_tip);
        setBlockIndexCandidates.insert(parked_walk_tip->pprev);
        if (parked_walk_tip->pprev == to_mark_parked && (to_mark_parked->nStatus & BLOCK_PARKED)) {
            to_mark_parked->nStatus = (to_mark_parked->nStatus ^ BLOCK_PARKED) | BLOCK_PARKED_CHILD;
            m_blockman.m_dirty_blockindex.insert(to_mark_parked);
        }

        // Add candidate blocks
        auto candidate_it = candidate_blocks_by_work.lower_bound(parked_walk_tip->pprev->nChainWork);
        while (candidate_it != candidate_blocks_by_work.end()) {
            if (!CBlockIndexWorkComparator()(candidate_it->second, parked_walk_tip->pprev)) {
                setBlockIndexCandidates.insert(candidate_it->second);
                candidate_it = candidate_blocks_by_work.erase(candidate_it);
            } else {
                ++candidate_it;
        ...
        to_mark_parked = parked_walk_tip;
    }
    ...
    {
        LOCK(cs_main);
        if (m_chain.Contains(to_mark_parked)) {
            return false;
        }

        // Mark pindex as parked
        to_mark_parked->nStatus |= BLOCK_PARKED;
        m_blockman.m_dirty_blockindex.insert(to_mark_parked);
        setBlockIndexCandidates.erase(to_mark_parked);

        // Clean up setBlockIndexCandidates
        for (auto& [_, block_index] : m_blockman.m_block_index) {
            if (block_index.IsValid(BLOCK_VALID_TRANSACTIONS) && !block_index.IsParked() &&
                block_index.HaveNumChainTxs() && !setBlockIndexCandidates.value_comp()(&block_index, m_chain.Tip())) {
                setBlockIndexCandidates.insert(&block_index);
            }
        }

        LogPrintf("BCH2: Parked block %s at height %d\n", to_mark_parked->GetBlockHash().ToString(), to_mark_parked->nHeight);
    }

    // Notify about tip change if needed
    if (pindex_was_in_chain) {
        (void)m_chainman.GetNotifications().blockTip(GetSynchronizationState(m_chainman.IsInitialBlockDownload()), *to_mark_parked->pprev);
    }
    return true;
}
...
void Chainstate::UnparkBlock(CBlockIndex* pindex)
{
    ...
    // Remove the parked flag from this block and all its descendants.
    for (auto& [_, block_index] : m_blockman.m_block_index) {
        if (block_index.IsParked() && block_index.GetAncestor(nHeight) == pindex) {
            block_index.nStatus &= ~BLOCK_PARKED_MASK;
            m_blockman.m_dirty_blockindex.insert(&block_index);
            if (block_index.IsValid(BLOCK_VALID_TRANSACTIONS) && block_index.HaveNumChainTxs() &&
    ...
    // Remove the parked flag from all ancestors too.
    while (pindex != nullptr) {
    ...
    LogPrintf("BCH2: Unparked block and descendants at height %d\n", nHeight);
}

void Chainstate::UnparkBlockAndDescendants(CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
    UnparkBlock(pindex);
}
...
bool Chainstate::FinalizeBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    ...
    // Can't finalize the genesis block
    if (pindex->nHeight == 0) {
        state.Invalid(BlockValidationResult::BLOCK_CACHED_INVALID, "bad-finalize-genesis", "Cannot finalize genesis block");
        return false;
    }

    // Block must be in the active chain
    if (!m_chain.Contains(pindex)) {
        state.Invalid(BlockValidationResult::BLOCK_CACHED_INVALID, "bad-finalize-not-in-chain", "Block to finalize is not in the active chain");
        return false;
    }

    // Update finalized height
    if (pindex->nHeight > m_finalized_height) {
        m_finalized_height = pindex->nHeight;
        m_blockman.m_block_tree_db->WriteFinalizedHeight(m_finalized_height);
        LogPrintf("BCH2: Finalized block %s at height %d\n", pindex->GetBlockHash().ToString(), m_finalized_height);

        // Park any competing chains that don't include this block
        for (auto& [_, block_index] : m_blockman.m_block_index) {
            // Skip blocks in the active chain
            if (m_chain.Contains(&block_index)) continue;

            // Skip already parked/invalid blocks
            if (block_index.IsParked() || !block_index.IsValid()) continue;

            // Skip blocks that have no data
            if (!block_index.HaveNumChainTxs()) continue;

            // If this block's ancestor at finalized height is NOT our finalized block,
            // park this competing block
            const CBlockIndex* ancestor = block_index.GetAncestor(m_finalized_height);
            if (ancestor != pindex) {
...
bool Chainstate::WouldCrossFinalization(const CBlockIndex* pindexFork) const
{
    AssertLockHeld(cs_main);

    // No finalization yet - allow all reorgs
    if (m_finalized_height < 0) return false;

    // If fork point is at or below finalized height, this would cross finalization
    if (pindexFork && pindexFork->nHeight <= m_finalized_height) {
        return true;
    }
...
void Chainstate::UpdateAutomaticFinalization(const CBlockIndex* pindexNew)
{
    AssertLockHeld(cs_main);

    // Get maxReorgDepth from consensus params
    const Consensus::Params& params = m_chainman.GetParams().GetConsensus();
    if (params.maxReorgDepth <= 0) {
        // Automatic finalization disabled
        return;
    }

    // Only update finalization after BCH2 fork is active
    if (!params.IsBCH2ForkActive(pindexNew->nHeight)) {
        return;
    }

    // Calculate the block that should be finalized
    // (current tip minus maxReorgDepth)
    int finalizeHeight = pindexNew->nHeight - params.maxReorgDepth;
    if (finalizeHeight <= 0) return;  // Don't finalize genesis

    // Only finalize if this would advance the finalized height
    if (finalizeHeight > m_finalized_height) {
        const CBlockIndex* pindexFinalize = pindexNew->GetAncestor(finalizeHeight);
        if (pindexFinalize && m_chain.Contains(pindexFinalize)) {
            m_finalized_height = finalizeHeight;
            LogPrintf("BCH2: Auto-finalized block at height %d (depth %d from tip %d)\n",
                      m_finalized_height, params.maxReorgDepth, pindexNew->nHeight);
        }
    }
}

// =========================================================================
// End BCH2/BCHN Automatic Finalization
// =========================================================================
...
void ChainstateManager::ReceivedBlockTransactions(const CBlock& block, CBlockIndex* pindexNew, const FlatFilePos& pos)
{
    ...
    if (GetConsensus().IsSegwitActive(pindexNew->nHeight)) {
 ...
 *
 * fork activation, witness commitments are not required and blocks have no
 * witness data. */
...
void ChainstateManager::UpdateUncommittedBlockStructures(CBlock& block, const CBlockIndex* pindexPrev) const
{
    ...
    if (commitpos != NO_WITNESS_COMMITMENT && GetConsensus().IsSegwitActive(nNextHeight) && !block.vtx[0]->HasWitness()) {
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    if (consensusparams.IsBCH2ForkActive(nHeight)) {
        // After BCH2 fork, no witness data is allowed
        for (const auto& tx : block.vtx) {
            if (tx->HasWitness()) {
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "unexpected-witness",
                    "SegWit transactions not allowed after BCH2 fork");
            }
        }
        // Also reject witness commitments in coinbase
        int commitpos = GetWitnessCommitmentIndex(block);
        if (commitpos != NO_WITNESS_COMMITMENT) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "unexpected-witness-commitment",
                "Witness commitment not allowed after BCH2 fork");
        }

        // After Magnetic Anomaly activation, transactions must be in canonical order:
        // - Coinbase first (already enforced by CheckBlock)
        // - All other transactions sorted by txid in lexicographical order
        if (consensusparams.IsMagneticAnomalyActive(nHeight) && block.vtx.size() > 1) {
                ...
                if (prevTx != nullptr) {
                    if (tx.GetHash() < prevTx->GetHash()) {
                        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "tx-ordering",
                            "transactions are not in canonical order (CTOR)");
                    }
                    if (tx.GetHash() == prevTx->GetHash()) {
        ...
        // Pre-fork: Standard witness validation
        ...
        if (!CheckWitnessMalleation(block, consensusparams.IsSegwitActive(nHeight), state)) {
            return false;
        }
    }

    // Check block size against the applicable limit.
    // Pre-fork: fixed legacy size limit.
    // Post-fork with ABLA (Upgrade 11): dynamic limit computed from ABLA state.
    // ABLA's minimum limit equals the initial 32MB, so this is always >= BCH2_MAX_BLOCK_SIZE.
    uint64_t nMaxBlockSize;
    if (consensusparams.IsBCH2ForkActive(nHeight)) {
        if (consensusparams.IsUpgrade11Active(nHeight) && pindexPrev != nullptr
            && pindexPrev->nABLAControlBlockSize > 0) {
            // Use cached ABLA state from the previous block
            nMaxBlockSize = pindexPrev->nABLAControlBlockSize + pindexPrev->nABLAElasticBufferSize;
            // Cap at 2GB (ABLA hard cap)
            nMaxBlockSize = std::min(nMaxBlockSize, MAX_CONSENSUS_BLOCK_SIZE);
        } else {
            // ABLA state not yet cached or pre-Upgrade 11: use initial ABLA limit (32MB)
            abla::Config ablaConfig = abla::Config::MakeDefault(consensusparams.nDefaultConsensusBlockSize);
            nMaxBlockSize = ablaConfig.epsilon0 + ablaConfig.beta0;
        }
    } else {
        nMaxBlockSize = BCH2_LEGACY_MAX_BLOCK_SIZE;
    }
    if (static_cast<uint64_t>(GetBlockWeight(block)) > nMaxBlockSize) {
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-blk-weight", strprintf("%s : weight limit failed (max=%llu)", __func__, nMaxBlockSize));
    }
...
bool Chainstate::LoadChainTip()
{
    ...
    // BCH2: Restore finalization height on startup.
    // Without this, m_finalized_height stays -1 after restart, allowing deep
    // reorgs past what should be finalized blocks.
    const Consensus::Params& csParams = m_chainman.GetConsensus();
    if (csParams.maxReorgDepth > 0 && csParams.IsBCH2ForkActive(tip->nHeight)) {
        int finalizeHeight = tip->nHeight - csParams.maxReorgDepth;

        // Check for persisted explicit finalization (may be higher)
        int persistedHeight = -1;
        if (m_blockman.m_block_tree_db->ReadFinalizedHeight(persistedHeight) &&
            persistedHeight > 0 && persistedHeight <= tip->nHeight) {
            finalizeHeight = std::max(finalizeHeight, persistedHeight);
        }

        if (finalizeHeight > 0) {
            m_finalized_height = finalizeHeight;
            LogPrintf("BCH2: Restored finalization height to %d on startup\n", m_finalized_height);
        }
    }
...
bool Chainstate::NeedsRedownload() const
{
    ...
    while (block != nullptr && consensusParams.IsSegwitActive(block->nHeight)) {
...
bool ChainstateManager::LoadBlockIndex()
{
    ...
    if (!fReindex) {
        ...
        for (CBlockIndex* pindex : vSortedByHeight) {
            ...
            // here.
            // BCH2/BCHN: Skip parked blocks when loading candidates
...
void ChainstateManager::CheckBlockIndex()
{
    ...
    CBlockIndex* pindexFirstParked = nullptr; // BCH2/BCHN: Oldest ancestor of pindex which is parked.
    while (pindex != nullptr) {
        nNodes++;
        if (pindexFirstAssumeValid == nullptr && pindex->nStatus & BLOCK_ASSUMED_VALID) pindexFirstAssumeValid = pindex;
        if (pindexFirstInvalid == nullptr && pindex->nStatus & BLOCK_FAILED_VALID) pindexFirstInvalid = pindex;
        if (pindexFirstParked == nullptr && (pindex->nStatus & BLOCK_PARKED)) pindexFirstParked = pindex;
        ...
        // BCH2/BCHN: Parked blocks consistency check
        if (pindexFirstParked == nullptr) {
            assert((pindex->nStatus & BLOCK_PARKED_MASK) == 0); // The parked mask cannot be set for blocks without parked ancestors.
        }
        ...
        for (auto c : GetAll()) {
            ...
            if (!CBlockIndexWorkComparator()(pindex, c->m_chain.Tip()) && pindexFirstNeverProcessed == nullptr) {
                if (pindexFirstInvalid == nullptr) {
                    ...
                    // setBlockIndexCandidates or be parked (BCH2/BCHN).
                    //
                    if ((pindexFirstMissing == nullptr || pindex == c->m_chain.Tip())) {
                        ...
                        // BCH2/BCHN: Parked blocks are removed from candidates, so allow them.
                        if (is_active || GetSnapshotBaseBlock()->GetAncestor(pindex->nHeight) == pindex) {
                            assert(pindexFirstParked != nullptr || c->setBlockIndexCandidates.count(pindex));
                        }
                    }
    ...
    const SnapshotMetadata& metadata)
{
    ...
    for (int i = AFTER_GENESIS_START; i <= snapshot_chainstate.m_chain.Height(); ++i) {
        ...
        if (GetConsensus().IsSegwitActive(index->nHeight)) {
