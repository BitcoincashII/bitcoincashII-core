...
```cmd
msbuild build_msvc\bitcoinII.sln -property:Configuration=Release -maxCpuCount -verbosity:minimal
```

Alternatively, open the `build_msvc/bitcoinII.sln` file in Visual Studio.
...
To check if `bitcoinIId` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers src/bitcoinIId.exe
```
